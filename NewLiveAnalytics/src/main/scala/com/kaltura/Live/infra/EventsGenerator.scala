...
import com.borhan.Live.model.dao.{BatchIdCF, LoggedFile, LoggedFilesCF}
import com.borhan.Live.model.parse.LiveEventParser
...
//     val session = cluster.connect(EnvParams.borhanKeySpace)
     ...
     def getLastBatchId()
     {
          //val rdd: SchemaRDD = cc.sql("SELECT * from borhan_live.log_files WHERE batch_id=-1")
     ...
     {
          //val rdd: SchemaRDD = cc.sql( "SELECT file_id from borhan_live.log_files WHERE batch_id=-1 ORDER BY insert_time LIMIT " + maxProcessFilesPerCycle.toString )
