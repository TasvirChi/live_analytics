package com.borhan.Live.model.purge

import java.util.Date

import com.borhan.Live.infra.ConfigurationManager
...
class DataCleaner(sc: SparkContext) extends Serializable with MetaLog[BaseLog] {
  ...
  def removeProcessedLogFiles() {
    ...
    val processedFilesRows = sc.cassandraTable(Consts.BorhanKeySpace, "log_files")
      ...
      connector.withSessionDo {
        session => {
          partition.foreach { row =>
            session.execute(s"DELETE FROM ${Consts.BorhanKeySpace}.log_data where file_id='${row.getString(0)}';")
            session.execute(s"DELETE FROM ${Consts.BorhanKeySpace}.log_files where file_id='${row.getString(0)}';")
          }
        }
      }
    })
  ...
  def removeLiveEvents() {
    ...
    val liveEventRows = sc.cassandraTable(Consts.BorhanKeySpace, "live_events")
    ...
    liveEventRows.foreachPartition(partition => {
      connector.withSessionDo {
        session => {
          partition.foreach { row =>
            session.execute(s"DELETE FROM ${Consts.BorhanKeySpace}.live_events where entry_id='${row._1}' and event_time='${row._2}';")
            session.execute(s"DELETE FROM ${Consts.BorhanKeySpace}.live_events_location where entry_id='${row._1}' and event_time='${row._2}';")
          }
        }
      }
    })
