...
import com.borhan.Live.model.dao.LiveEvents
...
object PeakAudienceNewProcessor {
  ...
  def process(sc: SparkContext, date: Long): Unit = {
    ...
    val liveNow = sc.cassandraTable("borhan_live", "hourly_live_events").select("entry_id").where("event_time = ?", startHour)
    val hourlyPeak = sc.cassandraTable("borhan_live", "live_entry_hourly_peak").select("entry_id", "event_time", "update_time", "audience", "dvr_audience").where("event_time = ?", startHour)
    ...
    allEntries.map(x => {
      ...
      .saveToCassandra("borhan_live", "live_entry_hourly_peak", toSomeColumns(entryPeakFieldsList))
  }
}
