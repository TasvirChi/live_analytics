Installation guide:
-------------------
1. Copy the configuration file from : BorhanLiveInfra/configuration/config.properties
	to a path of your choice, and expose it as a system variable named BORHAN_CONF_PATH
...
6. Verify web service was deplyed succesfully by entering - http://<tom-cat host>:<tomcat port>/BorhanLiveAnalytics/BorhanLiveAnalytics
7. Set the host name and execute the test client.

System variables:
----------------------
In order to expose a system variable, create a file named - /etc/profile.d/borhan.sh
and add the following content:
BORHAN_CONF_PATH=@CONFIGURATION_DIR@
export BORHAN_CONF_PATH
