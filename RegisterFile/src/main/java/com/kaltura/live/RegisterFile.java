...
import com.borhan.live.infra.exception.BorhanInternalException;
...
public class RegisterFile {
    ...
    public void insertIntoTable(String key, byte[] data) {
    	try {
	    	...
	    	PreparedStatement statement = cassandraSession.getSession().prepare("INSERT INTO borhan_live.log_data (file_id,data) VALUES (?, ?)");
	    	BoundStatement boundStatement = new BoundStatement(statement);
	        cassandraSession.execute(boundStatement.bind(key,ByteBuffer.wrap(data)), RETRIES);
	    	statement = cassandraSession.getSession().prepare("INSERT INTO borhan_live.log_files (file_id,insert_time,batch_id) VALUES (?, ?, ?)");
    ...
    public byte[] readFromTable(String key) {
        String q1 = "SELECT * FROM borhan_live.log_data WHERE id = '"+key+"';";
